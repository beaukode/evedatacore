// From https://github.com/latticexyz/mud/blob/f1751e3c2a8a7128867dc4624049d088d014e66a/packages/explorer/src/app/(explorer)/api/utils/decodeTable.ts
...
export const decodeTable = ({
  ...
  const keyNames = decodeAbiParameters(
    parseAbiParameters("string[]"),
    abiEncodedKeyNames as Hex
  )[0];
  const fieldNames = decodeAbiParameters(
    parseAbiParameters("string[]"),
    abiEncodedFieldNames as Hex
  )[0];

  const valueAbiTypes = [
    ...solidityValueSchema.staticFields,
    ...solidityValueSchema.dynamicFields,
  ];
  const keySchema = Object.fromEntries(
    solidityKeySchema.staticFields.map((abiType, i) => [
      keyNames[i],
      { type: abiType, internalType: abiType },
    ])
  ) satisfies Schema;
  const valueSchema = Object.fromEntries(
    valueAbiTypes.map((abiType, i) => [
      fieldNames[i],
      { type: abiType, internalType: abiType },
    ])
  ) satisfies Schema;
