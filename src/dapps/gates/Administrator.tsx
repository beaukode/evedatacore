...
const Administrator: React.FC = () => {
  ...
  const [corporation, setCorporation] = React.useState<number | null>(1000167);
  const { id } = useParams();
  const mudSql = useMudSql();

  const query = useQuery({
    queryKey: ["GateAccess", "Smartgate", id],
    queryFn: async () => mudSql.getAssembly(id || ""),
    enabled: !!id,
  });

  const queryCharacters = useQuery({
    queryKey: ["GateAccess", "Smartcharacters"],
  ...
  return (
    <Box m={2}>
      <PaperLevel1 title={title} loading={query.isLoading} backButton>
        {gate && (
          <>
